...
if ($this->UserIsAdmin()) {
    ...
    // test if the activite form exists, if not, install it
    checkAndAddForm($output, $GLOBALS['wiki']->config['lms_config']['activite_form_id'], ACTIVITE_FORM_NOM, ACTIVITE_FORM_DESCRIPTION, ACTIVITE_FORM_TEMPLATE);
    // test if the module form exists, if not, install it
    checkAndAddForm($output, $GLOBALS['wiki']->config['lms_config']['module_form_id'], MODULE_FORM_NOM, MODULE_FORM_DESCRIPTION, MODULE_FORM_TEMPLATE);
    // test if the parcours form exists, if not, install it
    checkAndAddForm($output, $GLOBALS['wiki']->config['lms_config']['parcours_form_id'], PARCOURS_FORM_NOM, PARCOURS_FORM_DESCRIPTION, PARCOURS_FORM_TEMPLATE);
    ...
    if (!$this->LoadPage('PageMenuLms')) {
        ...
        $this->SavePage('PageMenuLms', '""<div><span>""{{button link="config/root_page" nobtn="1" icon="fas fa-home"}}""</span><span style="float: right;">""{{button link="UserEntries" nobtn="1" text="Accès à mes fiches" icon="far fa-clone"></i>"}}""</span></div>""'
            . "\n\n" . '{{menuparcours}}');
        $output .= '✅ Done !<br />';
    } else {
