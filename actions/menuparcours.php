...
// show progress menu only if in activity view inside a module
//if (!empty($_GET['module'])) {
    ...
    $currentModule = getContextualModule(array_values($parcoursEntry)[0]);

    // display the menu only if a contextual parcours is found
    if (!empty($parcoursEntry)) {
        // only one parcours, get all modules for this parcours
        if (count($parcoursEntry) == 1) {
            $parcoursEntry = array_values($parcoursEntry)[0]; // take first array value
